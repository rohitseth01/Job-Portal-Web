    ...
    return (
        <div className="bg-white rounded-2xl">
            <Table>
                ...
                <TableBody>
                    {/* Safety: Use optional chaining on the array map */}
                        ...
                        <TableRow key={item?._id} className="hover:bg-gray-50/40 transition-colors">
                            ...
                            <TableCell>
                                {item?.applicant?.profile?.resume ? (
                                    <a href={item?.applicant?.profile?.resume} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-[#6A38C2] hover:underline font-medium group">
                                        <FileText size={16} className="text-gray-400 group-hover:text-[#6A38C2]" />
                                        <span className="truncate max-w-[150px]">{item?.applicant?.profile?.resumeOriginalName}</span>
                                    </a>
                                ) : (
                                ...
                                <Popover>
                                    ...
                                    <PopoverContent className="w-40 p-1 rounded-xl shadow-2xl border-gray-100">
                                        <div className="flex flex-col">
                                            <div onClick={() => statusHandler("Accepted", item?._id)} className="flex items-center gap-3 px-3 py-2 text-sm font-semibold text-green-600 hover:bg-green-50 rounded-lg cursor-pointer transition-all">
                                                <CheckCircle2 size={16} />
                                                <span>Shortlist</span>
                                            </div>
                                            <div onClick={() => statusHandler("Rejected", item?._id)} className="flex items-center gap-3 px-3 py-2 text-sm font-semibold text-red-600 hover:bg-red-50 rounded-lg cursor-pointer transition-all">
                                                <XCircle size={16} />
                                                <span>Reject</span>
                                            </div>
                                        </div>
